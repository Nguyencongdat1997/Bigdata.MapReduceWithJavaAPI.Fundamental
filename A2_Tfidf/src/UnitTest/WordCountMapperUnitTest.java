...
public class WordCountMapperUnitTest extends TestCase {
    ...
    public void testMultiWords() {
        ...
        try {
            out = driver.withInput(new LongWritable(0), new Text("foo@foofile\t25")).run();
        } catch (IOException ioe) {
            fail();
        }
 
        List<Pair<Text, Text>> expected = new ArrayList<Pair<Text, Text>>();
        expected.add(new Pair<Text, Text>(new Text("foofile"), new Text("foo=25")));
